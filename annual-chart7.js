...
$(document).ready(function() { 
    ...
    d3.csv( "https://health.data.ny.gov/resource/7v4g-avhe.csv?$select=year&$group=year&$order=year DESC", function(error, data) {
...
function preDraw() {
    ...
    $('<h2 class="pageHeading">(Page 46) Chart 7. Leading Underlying Causes of Death: New York State '+selectedYear+'</h2>').insertAfter('.selections');
...
function draw(selectedYear) {
    ...
    d3.csv("https://health.data.ny.gov/resource/7v4g-avhe.csv?$limit=50000&$select=*&$where=year ='"+selectedYear+"' and gender_description in ('Total') and age_group_description = 'Total' and race_ethnicity_description = 'Total' and selected_cause_of_death NOT IN ('Total')&$order=deaths DESC", function(error, data) {

        if (error) throw error;
        
        // rewrite data array so that '<1' age_group description is in first position
        // nest the data by selected_cause_of_death for the x axis
        var nestedAge = d3.nest()
            .key(function(d) { return d.selected_cause_of_death; })			 
            .entries(data);		

        var arraycount = $(nestedAge).toArray().length;  // one way to count elements in an object array
        var newcount = Object.keys(nestedAge).length;   // another way to count elements in an object array     
	
        // map all the county names from the query into an array
        var counties = d3.values(data).map(function(name){return name.selected_cause_of_death;});        
       ...
       /*  var nested_data = d3.nest()
        ...
        // var margin = {top: 40, right: 50, bottom: 20, left: 255};
        ...
        var leftmargin = fullwidth * 0.2;
        var rightmargin = fullwidth * 0.025;
        var topmargin = fullwidth * 0.025;
        var bottommargin = fullwidth * 0.025;
        ...
        var x1Scale = d3.scaleLinear() //d3v4
            .domain([0,	d3.max(nestedAge, function(d) { return d3.max(d.values,function(d) {return +d.deaths;}); })]) // input
            .range([0, width]);  //output

        // sets the y scale for the population values
        var yScale = d3.scaleBand()  //d3v4
            .domain(counties) // input uses counties on x-axis groups
            .padding(0.2)
            .rangeRound([-(arraycount*2), height + arraycount]); // output d3v4 
        ...
        .html(function(d) {
            var commas = +d.deaths;	
				return "<center><span style='color:orange'>" + d.selected_cause_of_death + "</span><br><strong>Deaths:</strong> <span style='color:SpringGreen'>" + commas.toLocaleString() + "</span></center>";
        })
        ...
        svg_bar.append("g")			
            .attr("transform", "translate(0," + (height + 10) + ")")  // the + 10 moves the xaxis labels down lower
//			.attr("class", "axis") // comment this line if using grid  
        ...
        svg_bar.append("g")
			.attr("class", "axis") // comment this line if using grid
 //           .attr("class", "grid") // uncomment this line if using 
...
//            .tickSize(-width) // uncomment this line if using grid
        ...
        var groupings = svg_bar.selectAll(".groupings")
            ...
            .append("g")
               ...
               .attr("transform", function(d,i) { return "translate(0 ," + i * (height / arraycount) +")"; });             
        groupings.selectAll("rect")
                ...
                .attr("y", function(d) { return yScale(d.counties); })
				.attr("width",function(d) { return x1Scale(+d.deaths);})
                .attr("fill", "steelblue")
                .on('mouseover', tip.show)
                .on('mouseout', tip.hide);
                
                
        groupings.selectAll(".text")  		
	  .data(function(d) {return d.values; })
	  .enter()
	  .append("text")
	  .attr("class","label")
        .attr("x", 25)
//	  .attr("x", (function(d) { return x1Scale(+d.deaths) + x1Scale.rangeBand() / 2 ; }  ))
        .attr("y", function(d) { return yScale(d.counties); })
//	  .attr("y", function(d) { return yScale(d.quantity) + 1; })
	  .attr("dy", ".75em")
	  .text(function(d) { return replaceNumberWithCommas(+d.deaths); });     
